    ...
    def end_session(self, final_balance, reason="manual_shutdown"):
        ...
        if self.trades_data:
            ...
            for trade in self.trades_data:
                ...
                # Calculate R:R ratio if available
...
class BybitFuturesBot:
    ...
    def calculate_position_size(self, symbol: str, price: float, account_balance: float):
        """USDT-based position sizing - 0.1% × leverage × (leverage/2) per trade"""
        try:
            ...
            # Calculate position value: margin × leverage × (leverage/2)
            position_value_usdt = margin_per_trade * self.leverage * (self.leverage / 2)
    ...
    async def monitor_positions(self):
        """Monitor positions - NO TRAILING STOPS, just track and close on signal reversal or portfolio drawdown"""
        ...
        while self.monitoring_active:
            try:
                ...
                for pos in positions:
                    ...
                    # Only check for signal reversal, no trailing stops
    ...
    async def place_bracket_order(self, trade_info: Dict, amount: float):
        ...
        try:
            # Set leverage first (try, but don't fail if already set)
            try:
                ...
                # If leverage is already set, ignore the error
            ...
            # Blacklist symbols with specific errors
            if any(x in error_msg for x in ['leverage invalid', 'not support', 'not exist', 'invalid symbol']):
                self.failed_symbols.add(symbol)
                self.logger.warning(f"Blacklisted {symbol} due to: {e}")
                return False
            
            # Try simple order without SL/TP
            try:
    ...
    async def run_trading_cycle(self):
        ...
        try:
            ...
            if long_slots == 0 and short_slots == 0:
                self.logger.info(f"At capacity (10/10)")
                return
            ...
            # Calculate uniform position size (all positions same USDT value)
            # 0.1% × leverage × (leverage/2) = 0.1% × 20 × 10 = 2% per trade
            uniform_margin = total_account * 0.001  # 0.1%
            ...
            for trade in selected_longs:
                try:
                    # Calculate amount based on uniform position value
                    ...
                    if amount < min_amount:
                        self.logger.warning(f"{trade['symbol']}: Amount {amount} < min {min_amount}, skipping")

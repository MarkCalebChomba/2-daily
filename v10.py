...
class EmailNotifier:
    ...
    def send_periodic_report(self, stats: dict):
        """Send 12-hour progress report"""
        try:
            if (datetime.now() - self.last_report_time).total_seconds() < 43200:  # 12 hours
                return False
...
class OKXFuturesBot:
    def __init__(self, api_key: str, api_secret: str, passphrase: str, test_mode: bool = False):
        ...
        self.breakeven_positions = set()  # Track positions at breakeven
    ...
    async def close_position(self, symbol: str, position_data: dict, reason: str):
        ...
        try:
            ...
            # Remove from breakeven set if it was there
        ...
        try:
            ...
            for p in positions:
                if p.get('contracts', 0) > 0:
                    ...
                    # Skip positions at breakeven - they don't count against limit
                    if symbol in self.breakeven_positions:
                        continue
            ...
            try:
                ...
                for pos in positions:
                    ...
                    if profit_alpha >= 3.5 and current_tier < 3:
                        ...
                        # Mark as breakeven - frees up slot for new trade
    ...
    async def run_trading_cycle(self):
        ...
        try:
            ...
            if datetime.now() - self.order_reset_time > timedelta(hours=1):
                ...
                # Don't clear positions - they continue running
                self.order_reset_time = datetime.now()
                await asyncio.sleep(5)
...
async def main():
    ...
    ║   • Continuous operation (no sessions)    ║
    ╚════════════════════════════════════════════╝
    """)
